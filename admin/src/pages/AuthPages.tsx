      ...
      <div className="auth-left">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, position: 'relative' }}>
          ...
          <div style={{ marginTop: 14, fontFamily: 'Outfit,sans-serif', fontSize: '.72rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)', display: 'flex', gap: 8 }}>
            <span style={{ color: 'rgba(127,214,122,.55)' }}>Track</span> ·
            <span style={{ color: 'rgba(127,214,122,.55)' }}>Monitor</span> ·
            <span style={{ color: 'rgba(127,214,122,.55)' }}>Harvest</span>
          </div>
        </div>
      </div>
...
export function ResetPasswordPage() {
  ...
  const [password, setPassword] = useState(''); const [done, setDone] = useState(false); const [error, setError] = useState(''); const [loading, setLoading] = useState(false)
  const submit = async (e: FormEvent) => {
    e.preventDefault(); setError(''); setLoading(true)
    ...
    <AuthShell>
      <div className="auth-card anim-up">
        {done ? (<><div style={{ fontSize: '2.5rem', marginBottom: 12 }}>✅</div><h2>Password updated</h2><p className="auth-sub">Your password has been reset.</p><Link to="/login" className="btn btn-primary" style={{ display: 'block', textAlign: 'center', marginTop: 20 }}>Sign In</Link></>) : (
          ...
          {error && <div className="alert alert-error" style={{ marginBottom: 16 }}>{error}</div>}
          <form className="auth-form" onSubmit={submit}>
            <div className="form-group"><label className="form-label">New Password</label><input className="form-input" type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={8}/></div>
