...
import { LayoutDashboard, Sprout, Users, Bell, Settings, LogOut, ChevronLeft, ChevronRight, Menu, X } from 'lucide-react'
...
export default function Sidebar({ unread, onNotifClick }: Props) {
  ...
  const nav = [
    ...
    { to: '/fields',    icon: <Sprout size={18}/>,          label: 'Fields' },
    { to: '/agents',    icon: <Users size={18}/>,            label: 'Agents' },
  ]
    ...
    <div className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      <div>
        ...
        <div className="sidebar-user">
          <Avatar initials={user?.avatar_initials||'?'} size={32} color="linear-gradient(135deg,#6C3FC5,#4e2d9a)"/>
          {!collapsed && <div style={{ overflow: 'hidden' }}>
            <span className="sidebar-user-name">{user?.name}</span>
            <span className="sidebar-user-role">Coordinator</span>
          </div>}
        </div>
      ...
      <div>
        ...
        <NavLink to="/profile"  title={collapsed ? 'Profile' : undefined}  className={({ isActive }) => `nav-item ${isActive?'active':''}`}>
          <span className="nav-icon"><Settings size={18}/></span><span className="nav-label">Settings</span>
        </NavLink>
        <button className="nav-item logout-item" onClick={() => { logout(); navigate('/login') }} title={collapsed ? 'Sign Out' : undefined}>
          <span className="nav-icon"><LogOut size={18}/></span><span className="nav-label">Sign Out</span>
        </button>
      </div>
    </div>
  )
